  ...
  Process {
    ...
    command: ["bash", "-lc",
      "if ! command -v omarchy-dock >/dev/null 2>&1; then " +
        ...
        "'This plugin is only the supervisor. Get the package from github.com/szlukabence/omarchy-dock/releases, or build it: cd ~/.config/omarchy/plugins/omarchy-dock && ./install.sh'; " +
        "exit 0; " +
      "fi; " +
